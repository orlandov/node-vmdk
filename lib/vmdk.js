  ...
  , function (callback) {
      self.vmdk.getMarker(offset, function (error, type, marker) {
        ...
        if (type === 'gt') {
          ...
          fs.read
          ( self.vmdk.fd, gtBuf, 0, 4*512, offset
          , function (error, bytesRead, buf) {
              var table = self.vmdk.parser.readData(GrainTableStruct, gtBuf, 0);
              console.warn("GTable contents");
              console.warn(util.inspect(table));
              offset = nextClosest(offset) + 4*512;
              return callback();
            }
          );
        }
          ...
          console.warn("lba was %d, output offset was %d", marker.lba[0]*512, outputOffset);
          fs.read
          ...
          , function (error, bytesRead, buf) {
              ...
              //console.warn("grain size was " + marker.size);
              ...
              , function (callback) {
                  ...
                  , function (error, bytesRead, buf) {
                      ...
                      gunzip.write(buf, function (error, decompressed) {
                        ...
                        //console.warn("decompressed size was " + decompressed.length);
